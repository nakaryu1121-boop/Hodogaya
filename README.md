...
<head>
    ...
    <title>HODOGAYA AREA DEFENSE - Ver. 9.0</title>
    ...
    <style>
        ...
        .symbol { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 2px solid #fff; border-radius: 4px; font-size: 14px; font-weight: 900; margin-right: 10px; }
...
<body class="p-4 md:p-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div class="lg:col-span-7 space-y-6">
                ...
                <div class="nerv-panel p-5">
                    <h2 class="text-[10px] font-black bg-[#E60012] px-2 py-0.5 text-white tracking-widest uppercase mb-4 w-fit">Transportation</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <a href="https://transit.yahoo.co.jp/diainfo/29/0" target="_blank" id="panel-yokosuka" class="bg-[#000] border border-[#222] p-4 flex items-center hover:bg-[#1a1a1a] transition">
                            <div class="symbol bg-[#0072bc]">JO</div>
                            <div class="flex-grow">
                                <p class="text-[8px] text-[#666] font-black uppercase">Yokosuka Line</p>
                                <span class="text-sm font-black">横須賀線 ≫</span>
                                <p id="status-yokosuka" class="text-[9px] font-bold mt-1 text-[#00ff41]">NORMAL</p>
                            </div>
                        </a>
                        <a href="https://transit.yahoo.co.jp/diainfo/25/0" target="_blank" id="panel-ssline" class="bg-[#000] border border-[#222] p-4 flex items-center hover:bg-[#1a1a1a] transition">
                            <div class="symbol bg-[#e21f26]">JS</div>
                            <div class="flex-grow">
                                <p class="text-[8px] text-[#666] font-black uppercase">S-S Line</p>
                                <span class="text-sm font-black">湘南新宿ライン ≫</span>
                                <p id="status-ssline" class="text-[9px] font-bold mt-1 text-[#00ff41]">NORMAL</p>
                            </div>
                        </a>
                    </div>
                </div>
    ...
    <script>
        ...
        // 2. 鉄道運行情報取得 (CORSプロキシ経由)
        async function checkTrainStatus() {
            const syncLabel = document.getElementById('sync-status');
            try {
                // AllOrigins プロキシを使用してCORSを回避
                const targetUrl = "https://tetsudo.rti-giken.jp/free/tetsudo_now/api/delay.json";
                const res = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(targetUrl)}`);
                const delays = await res.json();
                
                // 判定ロジック
                const yokosukaAlert = delays.some(d => d.name.includes("横須賀線"));
                const sslineAlert = delays.some(d => d.name.includes("湘南新宿ライン"));

                updatePanel("yokosuka", yokosukaAlert);
                updatePanel("ssline", sslineAlert);
                ...
                console.error("Transit Sync Fail:", e);
            }
        }
        ...
        function toggleTestAlert() {
            ...
            updatePanel("yokosuka", isTestAlert);
            updatePanel("ssline", isTestAlert);
            alert(isTestAlert ? "TEST_ALERT_ENABLED: 運行異常をシミュレートします。" : "TEST_ALERT_DISABLED: 通常状態に戻ります。");
        }
        ...
        // ループ設定
        setInterval(checkTrainStatus, 60000); // 1分
        setInterval(loadOverview, 600000);    // 10分
        setInterval(loadNerv, 300000);         // 5分
    </script>
</body>
</html>
